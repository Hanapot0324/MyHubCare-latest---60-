...
import { X, Bell, Calendar, Check } from 'lucide-react';
import { API_BASE_URL } from '../config/api';

const NotificationSystemPatient = ({ socket }) => {
    ...
    useEffect(() => {
        if (!socket) {
            console.warn('⚠️ Socket not available in NotificationSystemPatient');
            return;
        }
        ...
        const setupSocket = () => {
            if (socket.connected) {
                ...
                socket.on('connect', () => {
                    console.log('✅ Socket connected in NotificationSystemPatient');
                    joinUserRoom();
                    setupListeners();
                });
            }
    ...
    // Handle appointment confirmation (patient confirms appointment)
                        ...
                        <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
                            {notifications.length === 0 ? (
                                ...
                                notifications.map((notification) => {
                                    ...
                                    // Patients should only see "Confirm Appointment" button, never Accept/Decline
                                    // Only show confirm button if notification explicitly requires patient confirmation
                                    // and it's NOT a provider action notification
                                    const requiresConfirmation = notification.requires_confirmation && 
                                                    ...
                                                    {requiresConfirmation && (
                                                        <div style={{ marginTop: '12px', display: 'flex', gap: '8px' }}>
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    handleAppointmentConfirm(notification.appointment_id);
                                                                }}
                                                                disabled={processingAction === notification.appointment_id}
                                                                style={{
                                                                    padding: '6px 12px',
                                                                    background: processingAction === notification.appointment_id ? '#9ca3af' : '#10b981',
                                                                    color: 'white',
                                                                    border: 'none',
                                                                    borderRadius: '6px',
                                                                    cursor: processingAction === notification.appointment_id ? 'not-allowed' : 'pointer',
                                                                    fontSize: '12px',
                                                                    fontWeight: '600',
                                                                    display: 'flex',
                                                                    alignItems: 'center',
                    ...
                    {notification.type === 'appointment_declined' && (
                        <div style={{ marginBottom: '16px', padding: '12px', background: '#fef2f2', borderRadius: '8px', border: '1px solid #fecaca' }}>
                            ...
                            <p style={{ margin: 0, fontSize: '14px', color: '#991b1b' }}>
                                {notification.decline_reason || 'No reason provided'}
                            </p>
                        </div>
                    )}
...
export default NotificationSystemPatient;
